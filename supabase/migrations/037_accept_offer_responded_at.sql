-- ============================================================
-- Migration 037: set responded_at when cancelling competing offers in the
-- already_booked branch  (#18)
--   In accept_booking_offer's already_booked branch the sibling 'sent' offers
--   were cancelled without responded_at, leaving them NULL — inconsistent with
--   every other status transition in the function and with the accepted-path
--   competing-cancel. Admin "tidligere tilbud" history then sorts/shows them by
--   sent_at instead of the actual resolution time.
--
-- Identical to migration 031 except for the one UPDATE in the already_booked
-- branch, which now also sets responded_at = now().
-- ============================================================
