...
export function PublicHeader({ transparent, tone = 'dark', eventsHref = '/' }: { transparent?: boolean; tone?: 'dark' | 'light'; eventsHref?: string }) {
  ...
  const headerNav = (
    <nav className="fixed top-8 left-4 md:left-8 z-[9998] flex items-center gap-0 animate-fade-in" style={{ animationFillMode: 'both' }}>
      {/* Logo */}
  ...
  return (
    <>
      {createPortal(headerNav, document.body)}
